...
export function AddWorker({
	...
	const handleAddWorker = async () => {
		...
		if (!workerName) {
			setNameError(t("components.addWorker.workerNameCannotBeEmpty"));
			return;
		}

		if (!edit && workerList.find((worker: any) => worker.name === workerName)) {
			setNameError(t("components.addWorker.workerNameAlreadyExists"));
			return;
		}
		let mcpLocal: any = {};
					...
					{edit ? (
						...
						<Button onClick={() => setDialogOpen(true)} variant="ghost">
							...
							<span className="text-text-body text-[13px] leading-13 font-bold">
								{t("components.addWorker.newWorker")}
							</span>
						</Button>
					)}
				</DialogTrigger>
						...
						<DialogTitle className="m-0">
							<div className="flex gap-xs items-center justify-start">
								...
								<div className="text-base font-bold leading-10 text-text-action">
									{showEnvConfig
										? t("components.addWorker.configureMcpServer")
										: t("components.addWorker.addYourMcpServer")}
								</div>
								<CircleAlert size={16} />
							</div>
						</DialogTitle>
					...
					{showEnvConfig ? (
						...
						<>
							...
							<DialogFooter className="bg-white-100% !rounded-b-xl p-md">
								<Button
									...
									size="sm"
								>
									{t("components.addWorker.cancel")}
								</Button>
								<Button size="sm" onClick={handleConfigureMcpEnvSetting}>
									<span>{t("components.addWorker.configure")}</span>
									<ArrowRight size={16} />
								</Button>
							</DialogFooter>
						...
						<>
							<div className="flex flex-col gap-3 bg-white-100% p-md">
								...
								<div className="flex flex-col gap-sm ">
									<div className="text-text-body text-sm leading-normal font-bold">
										{t("components.addWorker.descriptionOptional")}
									</div>
									<Textarea
										placeholder=""
								...
								<div>
									<div className="flex items-center gap-sm ">
										<div className="text-text-body text-sm leading-normal font-bold">
											{t("components.addWorker.agentTool")}
										</div>
										<CircleAlert size={16} />
									</div>
								</div>
							...
							<DialogFooter className="bg-white-100% !rounded-b-xl p-md">
								...
								<Button size="sm" onClick={handleAddWorker} type="submit">
									<span>{t("components.addWorker.saveChanges")}</span>
								</Button>
							</DialogFooter>
						</>
					)}
