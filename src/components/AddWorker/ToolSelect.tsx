...
>(
	(
						...
						.map(([key, value]: [string, any]) => {
							...
							if (mcp) {
								onInstall = async () => {
									...
									if (!res.success) {
										toast.error(t("components.setting.installMCPFailed"), {
											closeButton: true,
										});
									}
								};
							...
							return {
								...
								desc:
									value.env_vars && value.env_vars.length > 0
										? `${t("components.toolSelect.environmentalVariablesRequired")} ${value.env_vars.join(
												", "
										  )}`
										: "",
								onInstall,
		...
		const getInstallButtonText = (itemId: number) => {
			if (installedIds.includes(itemId)) return t("components.toolSelect.installed");
			if (installing[itemId]) return t("components.toolSelect.installing");
			if (installed[itemId]) return t("components.toolSelect.installed");
			return t("components.toolSelect.install");
		};
			...
			>
				...
				<div className="flex items-center gap-1">
					<Button
						...
						disabled={true}
					>
						{t("components.toolSelect.installed")}
					</Button>
				</div>
			</div>
		);
