...
export const Permissions: React.FC = () => {
	...
	const [settings, setSettings] = useState([
		{
			title: t('components.permissions.enableScreenRecording'),
			checked: false,
		},
		{
			title: t('components.permissions.enableAccessLocalSoftware'),
			checked: false,
		},
		{
			title: t('components.permissions.grantLocationAccess'),
			checked: false,
		},
		{
			title: t('components.permissions.shareDataToEnhanceEigent'),
			checked: false,
		},
	]);
	useEffect(() => {
			...
			<div className="flex gap-md h-[568px] ">
				<div className=" flex flex-col gap-md w-[438px]">
					<div className="text-text-heading font-bold text-4xl leading-5xl">
						<div>{t('components.permissions.enablePermissions')}</div>
					</div>
					<div className="text-text-body text-xl leading-2xl font-medium">
						{t('components.permissions.grantPermissionToActivateAgent')}
					</div>
			...
			<div className="flex justify-end items-center gap-sm">
				...
				<div className="flex  justify-center items-center gap-sm">
					...
					<Button
						...
						size="sm"
					>
						<div>{t('components.permissions.next')}</div>
