		...
		<>
			...
			<div
				...
				className={`${data.isEditMode
					...
					} transition-all duration-300 ease-in-out ${data.agent?.tasks.length === 0 && "opacity-30"
					}`}
			>
				<div
					className={`py-2 px-3 pr-0 flex flex-col ${data.isEditMode ? "flex-1 min-w-[280px]" : "w-[280px] "
						}`}
				>
					...
					>
						...
						{agentToolkits[
							...
							data.agent?.tools
								...
								.filter(Boolean)
								.join(" ") ||
							t('components.workflowNode.noToolkits')}
					</div>
					<div
									...
									>
										<div className="pt-0.5">
											{task.status === "running" && (
												<LoaderCircle
													size={16}
													className={`text-icon-success ${chatStore.tasks[chatStore.activeTaskId as string]
				...
				{isExpanded && (
					<div
						...
						className={`${data.isEditMode ? "flex-1" : "w-[280px]"
							...
							{selectedTask?.report && (
								...
								>
									<div className="text-text-primary text-sm font-bold">
										{t('components.workflowNode.completionReport')}
									</div>
									<MarkDown
