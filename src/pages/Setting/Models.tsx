...
export default function SettingModels() {
	...
	const handleVerify = async (idx: number) => {
		...
		if (items[idx].id !== "local") {
			if (!apiKey || apiKey.trim() === "") {
				newErrors[idx].apiKey = t('components.setting.enterYourApiKey') + "!";
				hasError = true;
			} else {
		...
		if (!apiHost || apiHost.trim() === "") {
			newErrors[idx].apiHost = t('components.setting.enterYourUrl') + "!";
			hasError = true;
		} else {
			newErrors[idx].apiHost = "";
		}
		if (!model_type || model_type.trim() === "") {
			newErrors[idx].model_type = t('components.setting.enterYourModelType') + "!";
			hasError = true;
		} else {
		...
		try {
			...
			if (res.is_tool_calls && res.is_valid) {
				console.log("success");
				toast(t('components.setting.verify') + " " + t('common.success'), {
					description:
						t('components.setting.verifyModelSupportsFunctionCalling'),
					closeButton: true,
				});
			} else {
				console.log("failed", res.message);
				toast(t('components.setting.verify') + " " + t('common.failed'), {
					description: (
						<div
							style={{
								maxHeight: "120px",
							...
							<div className="flex justify-end">
								<Button
									...
									onClick={() => {
										...
										toast.success(t('components.setting.copiedToClipboard'));
									}}
								>
									{t('common.copy')}
								</Button>
	...
	const handleLocalVerify = async () => {
		...
		if (!localEndpoint) {
			setLocalError(t('components.setting.enterYourUrl') + "!");
		...
		try {
			...
			setLocalError(
			e.message || t('components.setting.verificationFailedPleaseCheckEndpointUrl')
		);
			setLocalInputError(true);
		} finally {
			setLocalVerifying(false);
		...
		<div className="space-y-2">
					...
					<div className="self-stretch flex flex-col justify-start items-start gap-1">
						<div className="self-stretch h-6 inline-flex justify-start items-center gap-2.5">
							<div className="flex-1 justify-center text-text-body text-base font-bold leading-snug">
								{t('components.setting.eigentCloudVersion')}
							</div>
							<Switch
								checked={cloudPrefer}
						...
						<div className="self-stretch justify-center">
							<span className="text-text-body text-xs font-normal font-['Inter'] leading-tight">
								{t('components.setting.youAreCurrentlySubscribedToThe')}{" "}
								{subscription?.plan_key?.charAt(0).toUpperCase() +
									subscription?.plan_key?.slice(1)}
								. {t('components.setting.discoverMoreAboutOur')}{" "}
							</span>
							<span
								onClick={() => {
							...
							>
								{t('components.setting.pricingOptions')}
							</span>
							<span className="text-text-body text-xs font-normal font-['Inter'] leading-tight">
								.
							</span>
					...
					<div className="w-full flex items-center flex-1 justify-between pt-4 border-t border-border-secondary">
						<div className="flex items-center flex-1 min-w-0">
							<span className="whitespace-nowrap overflow-hidden text-ellipsis text-xs font-medium leading-tight">
								{t('components.setting.selectModelType')}
							</span>
							<Tooltip>
								...
								>
									<span className="w-full flex items-center justify-center">
										{cloud_model_type === "gpt-4.1-mini"
											? t('components.setting.gpt41MiniDesc')
											: cloud_model_type === "gpt-4.1"
											? t('components.setting.gpt41Desc')
											: cloud_model_type === "claude-opus-4-1-20250805"
											? t('components.setting.claudeOpus41Desc')
											: cloud_model_type === "claude-sonnet-4-20250514"
											? t('components.setting.claudeSonnet4Desc')
											: cloud_model_type === "claude-3-5-haiku-20241022"
											? t('components.setting.claude35HaikuDesc')
											: cloud_model_type === "gpt-5"
											? t('components.setting.gpt5Desc')
											: cloud_model_type === "gpt-5-mini"
											? t('components.setting.gpt5MiniDesc')
											: cloud_model_type === "gpt-5-nano"
											? t('components.setting.gpt5NanoDesc')
											: t('components.setting.gemini25ProDesc')}
									</span>
								</TooltipContent>
							</Tooltip>
						</div>
							...
							>
								...
								<div className="flex w-full items-center gap-2">
									<div className="relative w-full">
										<div className="flex-1">
												...
												placeholder={`${t('components.setting.enterYourApiKey')} ${item.name} Key`}
								...
								<div className="mt-md space-y-4">
									<div>
																				<Input
											id={`apiHost-${item.id}`}
											placeholder={`${t('components.setting.enterYourUrl')} ${item.name}`}
											className="w-full"
									...
									<div>
										<Input
											...
											placeholder={`${t('components.setting.enterYourModelType')} ${item.name}`}
											className="w-full"
												...
												{ec.options && ec.options.length > 0 ? (
													...
													>
														<SelectTrigger className="bg-white-100% w-full border border-gray-200 rounded px-3 py-2 text-sm">
															<SelectValue placeholder={t('components.setting.pleaseSelect')} />
														</SelectTrigger>
								...
								<div className="flex justify-end mt-2">
									...
									>
										<span className="text-text-inverse-primary">
											{loading === idx ? "..." : t('components.setting.verify')}
										</span>
			...
			<div className="w-[630px] mt-4 px-6 py-4 bg-surface-secondary rounded-2xl flex flex-col gap-4">
				<div className="flex items-center justify-between mb-2">
					<div className="font-bold text-base">{t('components.setting.localModel')}</div>
					<Switch
						checked={localPrefer}
				...
				<div className="flex flex-col gap-3">
					<div>
						<label className="block text-sm font-bold mb-1">
							{t('components.setting.modelPlatform')}
						</label>
						<Select
							value={localPlatform}
							onValueChange={(v) => {
						...
						>
							<SelectTrigger className="w-full border border-solid border-border-primary bg-input-bg-default rounded px-3 py-2 text-sm">
								<SelectValue placeholder={t('components.setting.selectPlatform')} />
							</SelectTrigger>
							<SelectContent className="bg-white-100%">
					...
					<div>
						...
						>
							{t('components.setting.modelEndpointUrl')}
						</label>
						<Input
					...
					<div className="gap-1.5">
						<label className="block text-sm font-bold mb-1 leading-tight">
							{t('components.setting.modelType')}
						</label>
						<Input
							placeholder={t('components.setting.enterYourLocalModelType')}
							className="w-full"
							value={localType}
				...
				<div className="flex justify-end mt-2 ">
					...
					>
						<span className="text-text-inverse-primary">
							{localVerifying ? t('components.setting.verifying') : t('components.setting.verify')}
						</span>
						<Circle />
					</Button>
				</div>
			...
			>
				<DialogContent className="bg-white-100%">
					<DialogHeader>
						<DialogTitle>{t('components.setting.youAreOnSelfHostMode')}</DialogTitle>
					</DialogHeader>
					<DialogDescription className="space-y-2">
						<p className="indent-6">
							{t('components.setting.selfHostModeDesc1')}
						</p>
						<p className="indent-6">
							{t('components.setting.selfHostModeDesc2')}
						</p>
					</DialogDescription>
					<DialogFooter>
						<Button onClick={() => navigate("/setting/mcp")}>{t('common.close')}</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
		</div>
