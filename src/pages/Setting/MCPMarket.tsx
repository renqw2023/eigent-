	...
	return (
		<div className="h-full flex flex-col items-center ">
			...
			>
				...
				{t('components.mcpMarket.mcpMarket')}
			</span>
				</div>
				<div className="flex items-center gap-2 bg-white-100%">
					...
					>
						...
						>
							<span>{t('components.mcpMarket.all')}</span>
						</SelectItem>
							{mcpCategory.map((cat) => {
			...
			<div className="space-y-md w-full max-w-4xl overflow-y-auto scrollbar">
				{isLoading && items.length === 0 && (
					<div className="text-center py-8 text-gray-400">{t('components.mcpMarket.loading')}</div>
				)}
				{error && <div className="text-center py-8 text-red-500">{error}</div>}
				{!isLoading && !error && items.length === 0 && (
					<div className="text-center py-8 text-gray-400">{t('components.mcpMarket.noMCPServices')}</div>
				)}
				{items.map((item) => (
					<div
						key={item.id}
					...
					>
						...
						<div className="flex-1 min-w-0 flex flex-col justify-center">
							<div className="flex items-center gap-xs w-full pb-1">
								...
								>
																{installedIds.includes(item.id)
								? t('components.integrationList.uninstall')
								: installing[item.id]
								? t('components.toolSelect.installing')
								: installed[item.id]
								? t('components.integrationList.uninstall')
								: t('components.toolSelect.install')}
								</Button>
							</div>
							{item.home_page &&
				...
				<div ref={loader} />
				{isLoading && items.length > 0 && (
					<div className="text-center py-4 text-gray-400">{t('components.mcpMarket.loadingMore')}</div>
				)}
				{!hasMore && items.length > 0 && (
					<div className="text-center py-4 text-gray-400">
						{t('components.mcpMarket.noMoreMCPServers')}
					</div>
				)}
			</div>
		</div>
