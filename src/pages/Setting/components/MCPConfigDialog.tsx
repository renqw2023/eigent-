    ...
    <div className="fixed inset-0 z-30 flex items-center justify-center ">
      <div className="bg-white-100% rounded-lg shadow-lg p-6 min-w-[340px] max-w-[90vw] w-[480px] relative">
        <div className="flex items-center justify-between mb-2">
          <div className="font-bold">{t('components.mcpConfigDialog.editMCPConfig')}</div>
          <Switch
        ...
        <form onSubmit={onSave} className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1">{t('components.mcpConfigDialog.name')}</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t('components.mcpConfigDialog.description')}</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t('components.mcpConfigDialog.command')}</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t('components.mcpConfigDialog.args')}</label>
            <textarea
              autoComplete="off"
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t('components.mcpConfigDialog.env')}</label>
            {Object.entries(form.env).map(([k, v], idx) => (
              <div className="flex gap-2 mb-1" key={k + idx}>
                <input autoComplete="off" className="flex-1 border rounded px-2 py-1 text-xs" placeholder={t('components.mcpConfigDialog.key')} value={k} readOnly />
                <div className="relative flex-1">
                  <input 
                    autoComplete="off" 
                    className="w-full border rounded px-2 py-1 pr-8 text-xs" 
                    placeholder={t('components.mcpConfigDialog.value')} 
          ...
          <div className="flex justify-end mt-4 gap-2">
            <Button size="md" type="button" variant="outline" onClick={onClose} disabled={loading}>{t('components.mcpConfigDialog.cancel')}</Button>
            <Button size="md" type="submit" disabled={loading}>{t('components.mcpConfigDialog.save')}</Button>
          </div>
        </form>
      </div>
    </div>
