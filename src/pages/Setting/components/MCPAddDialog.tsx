	...
	React.useEffect(() => {
		if (open && localJson) {
			try {
				...
				setJsonError(t('components.mcpAddDialog.jsonFormatError') + (e.message || e.toString()));
			}
		} else if (open) {
			setJsonError(null);
		}
			...
			<DialogContent className="min-w-[340px] w-[600px] max-w-[95vw] p-0">
				...
				<div className="px-md py-md bg-white-100% rounded-b-xl">
					<div className="mb-4 text-sm text-gray-600 rounded-xl">
						...
						>
							{t('components.mcpAddDialog.learnMore')}
						</a>
					</div>
					{jsonError && (
					...
					<DialogFooter className="flex justify-end gap-2 mt-2 rounded-b-xl pt-sm">
						<Button
							...
							size="sm"
						>
							{installing ? t('components.mcpAddDialog.installing') : t('components.mcpAddDialog.install')}
